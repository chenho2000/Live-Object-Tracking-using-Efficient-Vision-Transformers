...
class CMT_ET_Tracker(Super_model_DP):
    ...
    def __init__(self, linear_reg=True, 
                    ...
                    temperature=1,
                    dropout=False):
        super(CMT_ET_Tracker, self).__init__()
        ...
        self.cls_branch_2 = cmt.Block(dim=outchannels_cls, num_heads=e_exemplars, mlp_ratio=3.6, qkv_bias=True)
        self.cls_branch_3 = cmt.Block(dim=outchannels_cls, num_heads=e_exemplars, mlp_ratio=3.6, qkv_bias=True)
        self.cls_branch_4 = cmt.Block(dim=outchannels_cls, num_heads=e_exemplars, mlp_ratio=3.6, qkv_bias=True)
        self.cls_branch_5 = cmt.Block(dim=outchannels_cls, num_heads=e_exemplars, mlp_ratio=3.6, qkv_bias=True)
        ...
        self.bbreg_branch_2 = cmt.Block(dim=outchannels_reg, num_heads=e_exemplars, qkv_bias=True)
        self.bbreg_branch_3 = cmt.Block(dim=outchannels_reg, num_heads=e_exemplars, qkv_bias=True)
        self.bbreg_branch_4 = cmt.Block(dim=outchannels_reg, num_heads=e_exemplars, qkv_bias=True)
        self.bbreg_branch_5 = cmt.Block(dim=outchannels_reg, num_heads=e_exemplars, qkv_bias=True)
        self.bbreg_branch_6 = cmt.Block(dim=outchannels_reg, num_heads=e_exemplars, qkv_bias=True)
        self.bbreg_branch_7 = cmt.Block(dim=outchannels_reg, num_heads=e_exemplars, qkv_bias=True)
    ...
    def forward(self, template, search, label=None, reg_target=None, reg_weight=None):
        ...
        #print(c.shape)
        c = self.cls_branch_2(c.view(batch, channel, height*width).permute(0, 2, 1), height, width, 1)
        c = self.cls_branch_3(c, height, width, 1)
        c = self.cls_branch_4(c, height, width, 1)
        c = self.cls_branch_5(c, height, width, 1)
        c = self.cls_branch_6(c.permute(0, 2, 1).view(batch, channel, height, width))
        c = self.cls_pred_head(c)

        b = self.bbreg_branch_1(feat_dict['reg'])
        batch, channel, height, width = b.shape
        b = self.bbreg_branch_2(b.view(batch, channel, height*width).permute(0, 2, 1), height, width, 1) 
        b = self.bbreg_branch_3(b, height, width, 1) 
        b = self.bbreg_branch_4(b, height, width, 1) 
        b = self.bbreg_branch_5(b, height, width, 1) 
        b = self.bbreg_branch_6(b, height, width, 1) 
        b = self.bbreg_branch_7(b, height, width, 1) 
        b = self.bbreg_branch_8(b.permute(0, 2, 1).view(batch, channel, height, width))
