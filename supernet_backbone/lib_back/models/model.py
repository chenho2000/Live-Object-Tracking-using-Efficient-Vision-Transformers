...
from supernet_backbone.lib_back.models.builder import *
...
def _gen_childnet(arch_list, arch_def, ops=None, **kwargs):
    ...
    for i, (layer_choice, layer_arch) in enumerate(zip(arch_list, arch_def)):
        #print(f'\n\ni: {i}')
        #print(f'layer choice: {layer_choice}')
        #print(f'layer arch: {layer_arch}')
        if len(layer_arch) == 1:
            ...
            for j, (block_choice, block_arch) in enumerate(zip(layer_choice, layer_arch)):
                #print(f'\nj: {j}')
                #print(f'block choice: {block_choice}')
                #print(f'block arch: {block_arch}')
                kernel_size, exp_ratio = choices_list[block_choice]
                #print(f'kernel size: {kernel_size}, exp_ratio: {exp_ratio}')
    ...
    #print(f'decoded architecture: {decode_arch_def(new_arch)}')
    model_kwargs = dict(
